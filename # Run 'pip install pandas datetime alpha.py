...
def validate_stock_name(stock):
    ...
    if(((stock.isalpha() == True) & ((len(stock) > 0 ) & (len(stock) < 6))) == true)
        ticketFlag = 1 #This means the data exist
        return ((stock.isalpha() == True) & ((len(stock) > 0 ) & (len(stock) < 6)))
    
    else
        print("The ticker symbol is not valid please check again\n")
        return -1 #ticket flag stays at 0 so it does not exist

# Validate chart type
def validate_chart_type(chartType):

    if((chartType == "1" or chartType == "2") == true)
        return
    
    else
        print("The number you selected is invalid. Please try again.")
        return -1 #try again if invallid number

# Validate date
def validate_time_series(timeSeries):

    if((timeSeries == "1" or timeSeries == "2" or timeSeries == "3" or timeSeries == "4") == true)
        return

    else
        print("The number you selected is invalid. Please try again.")
        return -1 #try again if invalid
